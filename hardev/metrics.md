...
| per-commit gate wall | **`--self` ≈ ¼ of full** `[M]` ✅ B2 SHIPPED (torajs runtimes only, drops 6 foreign; was ~10 min full 8-runner). artifact-precheck (→ seconds for tr-unchanged) = B2b next | (met early) B2b: seconds for tr-unchanged | seconds always; full cross-runtime only at phase-close `[D]` |
