...
use crate::layout::{ANY_HEAP, BUCKET_TAG_MASK, DEFINE_PRESENT_VALUE, TAG_ARR_HDR};
use crate::probe::{entries, entries_cap, entries_len, key_str_bytes, probe};
...
pub(crate) const FN_NAME_DELETED: u16 = 1 << 10;
pub(crate) const FN_LENGTH_DELETED: u16 = 1 << 11;

unsafe extern "C" {
    /// torajs-anyvalue — a closure cell's ES `length` metadata
    /// (`-1` = none: arrow / anon cells off the fn-addr registry).
    pub(crate) fn __torajs_closure_length(p: *mut c_void) -> i64;
    /// torajs-anyvalue — a closure cell's ES `name` as a fresh owned
    /// Str (anonymous answers `""`).
    pub(crate) fn __torajs_closure_name_str(p: *mut c_void) -> *mut u8;
}
...
/// install a throwing `constructor` getter ON THE INSTANCE).
...
pub(crate) unsafe fn define_apply(
    ...
    if pr.found {
        ...
        unsafe {
            crate::define_entry::define_fresh_entry(
                obj,
                key,
                pr.slot,
                tag,
