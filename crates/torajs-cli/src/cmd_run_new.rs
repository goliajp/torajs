//! `tr run` env-gated **new** pipeline — `TORAJS_NEW_PIPELINE=1`
//! mirrors `cmd_build_new`'s codegen + obj + link path, materializes
//! the binary to a unique temp file (pid + nanos), `exec`s it, and
//! cleans up. Equivalent to `tr build X.ts -o /tmp/X && /tmp/X` with
//! cleanup; conformance gate uses `tr run` so SD-4c-prereq atomic
//! cutover needs this before swap-C.
//!
...
use crate::cmd_build_new::{build_link_config, lower_to_ssa};
...
pub(crate) fn run(file_arg: Option<&String>) -> ExitCode {
    ...
    // Phase 0 step 8b — egraph mid-end pass (round-trip-equivalent
    // in Phase 0; `TORAJS_EGRAPH_OFF=1` bypasses for bisection).
