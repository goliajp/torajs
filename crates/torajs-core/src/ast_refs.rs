...
/// L3a-8 — width ground truth for an annotated-`number` fn param: does
/// any assignment to it inside the fn body have a statically-certain
/// f64 RHS (Div / fractional literal / Math.* — `infer_arg_width`'s
/// F64 set)? If so the param's I64-default slot must widen to F64 at
/// declaration, or the body's `n = n / 2` store hits the width-
/// mismatch reject. Two consumers MUST agree (K.3 lesson): the FnDecl
/// signature-collection loop (so call sites coerce i64 args via
/// SiToFp) and the body-lowering `param_setup` (so the slot itself is
/// F64). Synthetic fns (`__cm_*` / `__closure_*`) are excluded — their
/// call paths don't all consult the interned sig for arg coercion, so
/// widening only the callee side would split the ABI.
///
/// Misses are loud, not silent: an assignment shape this walk skips
/// (nested-fn capture writes, assigns hidden in nested expressions)
/// keeps the I64 slot and the existing lowering reject fires.
pub(crate) fn fn_param_widens_to_f64(ast: &Ast, fn_name: &str, body: &[Stmt], pname: &str) -> bool {
    if fn_name.starts_with("__") {
        return false;
    }
    body.iter().any(|s| stmt_assigns_f64(ast, s, pname))
}

fn stmt_assigns_f64(ast: &Ast, s: &Stmt, pname: &str) -> bool {
    use crate::ssa_lower::{NumWidth, infer_arg_width};
    let expr_is_f64_assign = |eid: ExprId| -> bool {
        if let Expr::Assign { target, value } = ast.get_expr(eid)
            && let Expr::Ident(n) = ast.get_expr(*target)
            && n == pname
        {
            return infer_arg_width(ast, *value) == NumWidth::F64;
        }
        false
    };
    match s {
        Stmt::Expr(e) => expr_is_f64_assign(*e),
        Stmt::If {
            then_branch,
            else_branch,
            ..
        } => {
            stmt_assigns_f64(ast, then_branch, pname)
                || else_branch
                    .as_ref()
                    .is_some_and(|eb| stmt_assigns_f64(ast, eb, pname))
        }
        Stmt::While { body, .. } | Stmt::DoWhile { body, .. } => stmt_assigns_f64(ast, body, pname),
        Stmt::For { step, body, .. } => {
            step.is_some_and(expr_is_f64_assign) || stmt_assigns_f64(ast, body, pname)
        }
        Stmt::ForOf { body, .. } | Stmt::ForOfSplitIter { body, .. } => {
            stmt_assigns_f64(ast, body, pname)
        }
        Stmt::Switch { cases, default, .. } => {
            cases
                .iter()
                .any(|c| c.body.iter().any(|cs| stmt_assigns_f64(ast, cs, pname)))
                || default
                    .as_ref()
                    .is_some_and(|d| d.iter().any(|ds| stmt_assigns_f64(ast, ds, pname)))
        }
        Stmt::Try {
            body,
            catch_body,
            ...
            body.iter().any(|bs| stmt_assigns_f64(ast, bs, pname))
                || catch_body.iter().any(|cs| stmt_assigns_f64(ast, cs, pname))
                || finally_body
                    .as_ref()
                    .is_some_and(|fb| fb.iter().any(|fs| stmt_assigns_f64(ast, fs, pname)))
        }
        Stmt::Block(v) | Stmt::Multi(v) => v.iter().any(|bs| stmt_assigns_f64(ast, bs, pname)),
        // Nested FnDecls have their own param scope; writes to an outer
        // param go through capture machinery, not this slot.
        _ => false,
    }
}
