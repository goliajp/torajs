...
use crate::ssa_lower_call_class_synth_proto as proto;
...
pub(crate) fn try_lower(
    ...
    let name = name.clone();
    match name.as_str() {
        "__torajs_proto_register" => proto::try_lower_proto_register(ctx, args),
        "__torajs_proto_link_fresh" => proto::try_lower_proto_link_fresh(ctx, args),
        ...
        "__torajs_class_computed_key" | "__torajs_to_property_key" => {
            reify::try_lower_class_computed_key(ctx, args)
        }
        "__torajs_obj_rest" => crate::ssa_lower_call_obj_rest::try_lower(ctx, args),
        // RFC 20260820-dstr-deferred-close — the dstr family lives in
        // the `_dstr` sibling (park / pending close / rest drain).
        "__torajs_dstr_close_pending" => {
            crate::ssa_lower_call_class_synth_dstr::try_lower_close_pending(ctx, args)
        }
        "__torajs_dstr_drain_rest" => {
            crate::ssa_lower_call_class_synth_dstr::try_lower_drain_rest(ctx, args)
        }
        ...
        "__torajs_super_prop_get" | "__torajs_super_prop_set" | "__torajs_super_prop_call" => {
