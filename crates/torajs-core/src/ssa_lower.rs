    ...
    fn lower_expr_inner(&mut self, eid: ExprId) -> Operand {
        ...
        match e {
            ...
            Expr::Call { callee, args } => {
                ...
                // / toString. See
                // [`crate::ssa_lower_call_namespace_obj_methods::try_lower`].
                if let Some(op) =
                    crate::ssa_lower_call_namespace_obj_methods::try_lower(self, *callee, args)
                {
                    return op;
                }
                // P3.struct-method-dispatch — `obj.method()` on
                // non-class `Type::Obj(sid)` struct whose named field
                // is FnSig or Closure. See
                // [`crate::ssa_lower_call_struct_method_dispatch::try_lower`].
                if let Some(op) =
                    crate::ssa_lower_call_struct_method_dispatch::try_lower(self, *callee, args)
                {
                    return op;
                }
                // V3-18 m2.a / m2.d — Object.prototype methods on
                // primitives (auto-boxing) AND struct instances. See
                // `ssa_lower_call_universal_methods` (chunk-15 of
                // Expr::Call decomp). Catch BEFORE any other Member-
                // method dispatch since these names overlap nothing
                // else. Arr.valueOf identity narrow + obj.toString
                // "[object Object]" stub + obj.hasOwnProperty/
                // propertyIsEnumerable compile-time/runtime str_eq
                // chain preserved.
                if let Some(op) =
                    crate::ssa_lower_call_universal_methods::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // Carve-out: see
                // [`crate::ssa_lower_call_number_methods::try_lower`]. Returns `None`
                // when callee shape doesn't match, so dispatch falls through to the
                // next arm (`Array.isArray(...)` and below).
                if let Some(op) =
                    crate::ssa_lower_call_number_methods::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // `isFinite`, `queueMicrotask` — see
                // `ssa_lower_call_bare_globals` (chunk-6 of Expr::Call decomp).
                if let Some(op) = crate::ssa_lower_call_bare_globals::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // decomp). 6 methods: parseInt / parseFloat / isInteger /
                // isNaN / isFinite / isSafeInteger.
                if let Some(op) =
                    crate::ssa_lower_call_number_namespace::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // `Array.of(...vals)` — emit no-spread array literal
                // SSA shape (arr_alloc(n) + len-store + slot stores).
                // See [`crate::ssa_lower_call_array_of::try_lower`].
                if let Some(op) = crate::ssa_lower_call_array_of::try_lower(self, *callee, args) {
                    return op;
                }
                ...
                // Three iter shapes: string / typed Array<T> / Set; optional
                // mapFn loop pushes `Array<R>` from each mapped element.
                if let Some(op) = crate::ssa_lower_call_array_from::try_lower(self, *callee, args) {
                    return op;
                }
                ...
                // S127-3 — `Object.assign(target, ...sources)` per ES
                // §20.1.2.1. See `ssa_lower_call_object_assign` (chunk-20
                // of Expr::Call decomp). Returns `None` when callee
                // isn't `Object.assign(...)` or args.is_empty().
                if let Some(op) =
                    crate::ssa_lower_call_object_assign::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // See `ssa_lower_call_object_values` (chunk-17 of
                // Expr::Call decomp). Returns `None` when callee isn't
                // `Object.values(...)` or args.is_empty().
                if let Some(op) =
                    crate::ssa_lower_call_object_values::try_lower(self, *callee, args)
                {
                    return op;
                }
                // ES §28.1.6 — `Reflect.get(target, key)` compile-time
                // fold extracted to `ssa_lower_call_reflect_get`
                // (chunk-22 of Expr::Call decomp). Returns `Some` on
                // typed struct + literal key (folds to field load +
                // box-to-Any); panics on non-struct target; falls
                // through (`None`) on shape mismatch.
                if let Some(op) = crate::ssa_lower_call_reflect_get::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // `__torajs_weakref_create`) extracted to
                // `ssa_lower_call_bun_runtime` (chunk-21 of Expr::Call
                // decomp). Returns `Some` when the callee matches one of
                // those forms; falls through otherwise.
                if let Some(op) = crate::ssa_lower_call_bun_runtime::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // T-17.a/.b/.c/.d + P10.2-A1 + T-15.g.1/.5 — `Promise.all` /
                // `.race` / `.any` / `.allSettled` sync fast paths,
                // 0-arg `Promise.{resolve,reject}()`, and 1+arg
                // `Promise.{resolve,reject}(v, ...)` (with T-19.f
                // thenable absorption + heap/primitive dispatch + ②.6b
                // F64 bit-cast) extracted to
                // `ssa_lower_call_promise_static` (chunk-27 of
                // Expr::Call decomp).
                if let Some(op) =
                    crate::ssa_lower_call_promise_static::try_lower(self, eid, *callee, args)
                {
                    return op;
                }
                // Object.* integrity / meta trap cluster (`freeze` /
                // `isFrozen` / `create` / `preventExtensions` /
                // `isExtensible` / `seal` / `isSealed` / no-op
                // `setPrototypeOf` / `defineProperties` / `assign`)
                // extracted to `ssa_lower_call_object_integrity`
                // (chunk-28 of Expr::Call decomp). The chunk-20
                // `ssa_lower_call_object_assign` arm runs first and
                // claims the structural-copy `Object.assign` path;
                // this falls through when that sibling misses.
                if let Some(op) =
                    crate::ssa_lower_call_object_integrity::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                if let Some(op) =
                    crate::ssa_lower_call_object_entries::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // `<Arr>.push(v)` in-place — 3 receiver shapes (Ident local /
                // K.8 global / `obj.field`): carve-out into
                // [`crate::ssa_lower_call_arr_push::try_lower`]. Other shapes
                ...
                // |difference|symmetricDifference}` carve-out: see
                // [`crate::ssa_lower_call_set_dispatch::try_lower`]. Returns `None`
                ...
                // .toISOString + 36 sibling Date methods). Recognized via
                // receiver type Type::Date; routed to the matching
                // `__torajs_date_*` runtime intrinsic with per-method
                // arg-shape handling (0-arg / 1-arg / per-field setters
                // with DATE_FIELD_KEEP padding). See
                // [`crate::ssa_lower_call_date_methods::try_lower`].
                if let Some(op) = crate::ssa_lower_call_date_methods::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // `ssa_lower_call_str_regex_methods` (chunk-7 of Expr::Call
                // decomp). Returns `Some` when the first arg is statically
                // a regex (literal or Ident with Type::RegExp); falls through
                // to the M6.1 String/Substr/Array stdlib sidekick otherwise.
                if let Some(op) =
                    crate::ssa_lower_call_str_regex_methods::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // `xs.findIndex|findLastIndex|find|findLast|some|every(p)`
                // short-circuit predicate iteration — see
                // `ssa_lower_call_arr_predicate` (chunk-10 of Expr::Call
                // decomp). findLastIndex / findLast walk back-to-front;
                // the rest walk front-to-back. All exit on first match
                // (find* / some) or first miss (every) per ES §23.1.3.{5-10}.
                if let Some(op) =
                    crate::ssa_lower_call_arr_predicate::try_lower(self, *callee, args)
                {
                    return op;
                }
                // `xs.flatMap(fn)` — outer loop over xs, per element
                // call `fn(elem)` to get an Array<T>, inner loop pushing
                // each into dst. Inner elem is rc_inc'd before push
                // (refcounted types only) so the inner array's drop
                // balances. See `ssa_lower_call_arr_flat_map` (chunk-11
                // of Expr::Call decomp). S319 widening: thisArg +
                // trailing args lowered-and-dropped for spec L-to-R
                // side-effect order; check.rs S270 already typecheck-
                // drops args[1..] for the Array callback family.
                if let Some(op) = crate::ssa_lower_call_arr_flat_map::try_lower(self, *callee, args)
                {
                    return op;
                }
                ...
                // generalized indirect (callee is a `Call`/`Closure`
                // expr — chained call `f(0)(5)`, IIFE `(() => v)()`).
                // See `ssa_lower_call_fn_indirect` (chunk-13 of
                // Expr::Call decomp). Non-callable Call/Closure callee
                // falls through to resolve_callee for a clearer panic.
                if let Some(op) = crate::ssa_lower_call_fn_indirect::try_lower(self, *callee, args)
                {
                    return op;
                }
            ...
            Expr::Array(elements) => {
                ...
                for eid in &element_ids {
                    if let Expr::Spread { expr } = self.ast.get_expr(*eid) {
                        ...
                        // `[...set]` reuses the Array.from(set) helper to
                        // walk the map-iter bucket chain into a fresh
                        // Arr<Any>; the result then flows through the same
                        // Arr-merge path that handles any other array
                        // spread source.
                        if matches!(v_ty, Type::Set) {
                            let arr_any_id = intern_arr_layout(self.arr_layouts, Type::Any);
                            v = crate::ssa_lower_arr_from_set::emit(self, v);
