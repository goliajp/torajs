...
pub(crate) fn try_route(
    ...
    if let Some(r) = super::route_fn_value::try_builtin_mv_fn_surface(checker, ast, callee, args) {
        return Some(r);
    }
    if let Some(r) = super::route_fn_value::try_builtin_mv_plain_call(checker, ast, callee, args) {
        return Some(r);
    }
