...
//!
//! Phase 0 (this commit) — substrate scaffold only:
//!
...
//!
//! Phase 0 explicitly DOES NOT mutate `Module::funcs`. `inline_module`
//! observes the module by shared reference; the SSA output is
//! bit-identical to the input. Phase 1 introduces an emit-side mutation
//! API (callee-body cloning + `Call` replacement + ValueId renumbering)
//! and switches `inline_module` to `&mut Module`. Until then, no
//! production driver wires this scaffold in — `transform_module` in
//! `lib.rs` is unchanged so conformance hold is automatic.
//!
//! Phase 0 also stops short of LoopAnalysis-scaled callee cost
//! (Cranelift / LLVM weight callee cost by call-site loop depth so a
//! hot-loop body inlines more eagerly than a one-shot init body). The
//! current `callee_body_cost` is the flat per-instruction sum — fine
//! for the scaffold because no emit decision actually fires. Phase 1
//! threads `LoopAnalysis` of the caller through `classify_caller_sites`.
