...
/// The buffer families' ids (and `Uint8Array`'s four text
/// conversions) live in [`crate::any_method_buffer`] — this file
/// reached the 500-line limit. They are re-exported here so the id
/// surface stays ONE import for every consumer, and so the shared,
/// append-only numbering is still read as a single space.
pub use crate::any_method_buffer::*;
