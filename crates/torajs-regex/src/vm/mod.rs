...
pub fn search_from_with_ws(
    ...
    // result depends only on the immutable program structure.
    //
    // Gate composition:
    ...
    //
    // Future chunks 8 (position-context state) and 9 (per-state save-mask)
    // remove ops from this gate as they land, widening the fast path.
