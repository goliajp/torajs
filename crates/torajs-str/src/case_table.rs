// CODEGEN: scripts/ucd/gen_case_tables.py — file-size audit carve-out (file-size.md #1).
//! UCD case-mapping tables — auto-generated by
//! `scripts/ucd/gen_case_tables.py` from UnicodeData.txt +
//! SpecialCasing.txt. DO NOT EDIT BY HAND.
//!
//! UCD version: 16.0.0
//!
//! Tables:
//! - `SIMPLE_UPPER` / `SIMPLE_LOWER`: source-cp -> 1-cp mapping
//! - `FULL_UPPER` / `FULL_LOWER`: source-cp -> N-cp expansion
//!   (SpecialCasing.txt Unconditional entries only)
//!
//! Final_Sigma (context-dependent `Σ -> ς`) is NOT in these
//! tables — see P11.5-A3 follow-up substrate work.
//!
//! All tables are sorted by source cp for binary search.

#![allow(clippy::unreadable_literal)]

#[allow(dead_code)]
pub(crate) const UCD_VERSION: &str = "16.0.0";

/// 1477 entries.
pub(crate) static SIMPLE_UPPER: &[(u32, u32)] = &[
    (0x0061, 0x0041),
    (0x0062, 0x0042),
    (0x0063, 0x0043),
    (0x0064, 0x0044),
...
/// 1460 entries.
pub(crate) static SIMPLE_LOWER: &[(u32, u32)] = &[
    (0x0041, 0x0061),
    (0x0042, 0x0062),
    (0x0043, 0x0063),
    (0x0044, 0x0064),
...
/// 102 entries (Unconditional SpecialCasing only).
pub(crate) static FULL_UPPER: &[(u32, &'static [u32])] = &[
...
/// 28 entries (Unconditional SpecialCasing only).
pub(crate) static FULL_LOWER: &[(u32, &'static [u32])] = &[
...
/// 159 inclusive ranges (sorted, merged).
pub(crate) static CASED_RANGES: &[(u32, u32)] = &[
    (0x0041, 0x005A),
    (0x0061, 0x007A),
    (0x00AA, 0x00AA),
    (0x00B5, 0x00B5),
...
/// 452 inclusive ranges (sorted, merged).
pub(crate) static CASE_IGNORABLE_RANGES: &[(u32, u32)] = &[
    (0x0027, 0x0027),
    (0x002E, 0x002E),
    (0x003A, 0x003A),
    (0x005E, 0x005E),
...
/// 34 inclusive ranges (sorted, merged).
pub(crate) static SOFT_DOTTED_RANGES: &[(u32, u32)] = &[
    (0x0069, 0x006A),
    (0x012F, 0x012F),
    (0x0249, 0x0249),
    (0x0268, 0x0268),
...
/// 131 inclusive ranges (sorted, merged).
pub(crate) static CCC_ABOVE_RANGES: &[(u32, u32)] = &[
    (0x0300, 0x0314),
    (0x033D, 0x0344),
    (0x0346, 0x0346),
    (0x034A, 0x034C),
...
/// 196 inclusive ranges (sorted, merged).
pub(crate) static CCC_NONZERO_RANGES: &[(u32, u32)] = &[
    (0x0300, 0x034E),
    (0x0350, 0x036F),
    (0x0483, 0x0487),
    (0x0591, 0x05BD),
...
/// Look up `cp` in a sorted `(u32, u32)` simple-mapping table.
/// Returns `Some(mapped)` if found, else `None`. Binary search
/// — O(log N) per lookup, N ~1500 per direction.
#[inline]
pub(crate) fn simple_lookup(table: &[(u32, u32)], cp: u32) -> Option<u32> {
    table
...
/// Look up `cp` in a sorted `(u32, &[u32])` full-mapping table.
/// Returns `Some(slice)` if found, else `None`.
#[inline]
...
/// Test whether `cp` carries the UCD `Cased` derived property.
...
/// Test whether `cp` carries the UCD `Case_Ignorable` derived
/// property — used by the Final_Sigma context check to skip over
/// non-significant code points between Σ and the surrounding Cased letters.
#[inline]
pub(crate) fn is_case_ignorable(cp: u32) -> bool {
    range_contains(CASE_IGNORABLE_RANGES, cp)
}

/// Test whether `cp` carries the UCD `Soft_Dotted` property
/// (PropList.txt) — the Lithuanian After_Soft_Dotted context rule.
#[inline]
pub(crate) fn is_soft_dotted(cp: u32) -> bool {
    range_contains(SOFT_DOTTED_RANGES, cp)
}

/// Test whether `cp` has Canonical_Combining_Class 230 (Above).
#[inline]
pub(crate) fn is_ccc_above(cp: u32) -> bool {
    range_contains(CCC_ABOVE_RANGES, cp)
}

/// Test whether `cp` has Canonical_Combining_Class 0 (starter).
#[inline]
pub(crate) fn is_ccc_zero(cp: u32) -> bool {
    !range_contains(CCC_NONZERO_RANGES, cp)
}

/// Shared binary search over a sorted inclusive-range table.
#[inline]
