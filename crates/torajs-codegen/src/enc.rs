...
//!
//! S3-A1 coverage: LDR/STR (immediate, unsigned offset, 64-bit X)
//! plus STP/LDP (pre/post-indexed, X-pair) for the prologue /
...
// --- S3-A1: load / store + pair load/store for prologue/epilogue ---

/// LDR Xt, [Xn, #imm12] — load 64-bit, unsigned offset, scaled by 8.
///
/// ARM ARM C6.2.131: `1 1 1 11001 01 imm12 Rn Rt`.
/// `byte_offset` must be 8-byte aligned and fit in 12 bits after
/// dividing by 8 (max 32760 byte = 4095*8).
...
///
/// ARM ARM C6.2.385 (pre-index variant). `byte_offset` is signed,
/// must be 8-aligned, fits in `imm7 * 8` range (-512..=504).
///
/// Used in prologue as `STP x29, x30, [SP, #-16]!` to push FP/LR
/// while pre-decrementing SP.
...
///
/// ARM ARM C6.2.151 (post-index variant). Mirror of `stp_pre_index`
/// for the epilogue: `LDP x29, x30, [SP], #16` pops FP/LR while
/// post-incrementing SP.
...
mod tests {
    ...
    fn ldr_x0_x9_offset_zero_matches_arm_arm() {
        // LDR x0, [x9, #0]: base 0xF940_0000, Rn=9, Rt=0
        //   = 0xF940_0120
    ...
    fn ldr_with_scaled_offset() {
        // LDR x0, [x9, #16] — imm12 = 16/8 = 2
        //   base 0xF940_0000 | (2<<10) | (9<<5) | 0
        //   = 0xF940_0820 | 0x0120
        //   = 0xF940_0920
    ...
    fn str_x9_x12_offset_zero_matches_arm_arm() {
        // STR x9, [x12, #0]: base 0xF900_0000, Rn=12, Rt=9
        //   = 0xF900_0189
    ...
    fn stp_x29_x30_pre_index_minus_16_matches_clang() {
        ...
        // Apple clang/gcc both emit this exact 32-bit word.
        //   = 0xA9BF_7BFD
