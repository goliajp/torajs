...
//! outputs).
//!
//! The early-exit predicate walks that share this callback ABI but
//! build no product — `every` / `some` / `find` / `findIndex` /
//! `findLast` / `findLastIndex` — live in
//! [`crate::method_any_predicate`].
...
pub(crate) type BoxedFn = unsafe extern "C" fn(*mut c_void, *const u64, i64) -> u64;

/// Adapter argv width mirror (`MAX_BOXED_PARAMS`).
pub(crate) const ARGV_SLOTS: usize = 8;

/// NaN-box `undefined`.
#[inline]
pub(crate) unsafe fn undef() -> u64 {
