...
65+ commits since `7c7844e` added zero perf regression on the scoreboard's stable cases — long cases (fib40, mandelbrot, gcd1m, collatz, prime_count, closure-counter) all sit within ±2% of their `7c7844e`-era values across every bench since. Conformance grew from 197 → **252 ports** — Math 100% complete, object spread, default + rest params + spread call args, JSON.stringify, multi-arg console, variadic String.fromCharCode, Number.toString(radix), generic class fixed-point monomorphization (`class Wrapper<T>`, `class Stack<T>`, `class Pair<A, B>`), `instanceof` (direct + extends-chain walk), and many integration ports. See [docs/100-percent-plan.md](docs/100-percent-plan.md) for the subset-expansion roadmap toward 100% test262 coverage.
