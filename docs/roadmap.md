...
| **M-OO.5** ◐ | public / private / protected / readonly modifiers — parser + AST metadata shipped (`Visibility` enum + `Ast.member_visibility` / `Ast.readonly_fields` maps); enforcement deferred. `Type::Struct` is structural-only today, so check.rs can't bridge `obj.member` back to the declaring class for visibility lookup. Same framing as M-OO.3 vtable upgrade: surface accepts the syntax, runtime / typecheck-time refinement waits on a substrate change (introducing `Type::Class(name, fields)` to retain nominal info through alias resolution). | shipped surface — `m-oo-05-modifiers-syntax.ts` exercises every modifier shape; enforcement tracked separately |
...
| **M6.3** ◐ | `Date`, `JSON.parse` / `JSON.stringify` | `JSON.stringify` shipped end-to-end for every torajs type with a sensible JSON encoding: Number / F64 / Bool / Str / Substr / Array / Object / Class-instance (the desugared class layout IS a struct, so the Object arm covers it transparently). `m6-09-json-stringify-primitives.ts` covers escape-table edge cases; `m6-10-json-stringify-arr-obj.ts` covers nested Array<Object> and mixed-type structs. `JSON.parse` deferred (needs `Type::Class` or tagged-union return shape since torajs has no `any`); `Date` not yet started. |
