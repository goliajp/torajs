...
key hash, the property-key spelling — 562 even changed a prototype's
own-key ORDER, which `getOwnPropertyNames` shows, and no case reads
it); 563-566 moved **11 cases**, every one of them out of the `bug`
bucket and into `pass` (565: one computed-member name; 566: eight
`built-ins/DataView` plus both `object/fn-name-accessor` files). The
predicate counts the `incompatible` face — what tr REFUSES — and none
of those eleven were refusals. So every count below is rotation 566's
own measurement, not a stale stamp: clusters **143**, cases **1130**,
...
- **S7-e 已并入 S7-d 的 P2**,随 P2 一同 ship。
  ...
  **全矩阵同轮 A/B(44 cell,`--runs 5`,`3763f4d81` vs `ea112112e`)**:
  `array-sum-1m` **−36.1%**(tr/rust 1.701 → **1.064**)·
  `generic-id-1m` **−26.4%**(2.072 → **1.503**,正是本节点名的抽象税
  cell)· `closure-pipeline-1m` **−14.2%**(1.257 → **1.058**)·
  `array-map-1m` −3.8%。中位 tr/bun-aot **0.494 → 0.476**。
  另有 12 个 cell 读数 +3~9%,**它们的产物 sha 逐字相同**,同轮 bun 中位
  +2.5% / rust +1.9% —— 机器整体变慢,不是代码。
  唯一另一个 ≥4% 的读数 `startup` +4.3% 的**产物逐字相同**(18425 B,
  同一 sha256)—— 同一个二进制不会因代码变慢,那是 spawn 测量噪声。
  `array-map-1m` 第一版**没动**,查因不是 allowlist 而是两端证明的名字
  普查太粗(`loopSum` 有两个各自 `for (let i …)` 的兄弟循环);`ea112112e`
  收紧成「循环自己绑定的计数器外面没有这个名字可写」后它也窄化了。
  **这条精度损失只有全矩阵 A/B 看得见。** 同理它从 1.857 掉到 1.459 是
  rotation 535 的 P0 干的,当轮只量了 `array-sum-1m` 就记了「墙钟不动」,
  那 −20% 因此漏记 —— **codegen 面的刀当轮不跑全矩阵,收益会记到下一轮
  头上。**
