...
- [x] **V3-04** Karatsuba multiplication — shipped `f192fd4` (378/0/1). 32-limb threshold; schoolbook stays for small. Recursive divide-and-conquer with 3 sub-products via the `(xl+xh)(yl+yh) - z0 - z2` identity. Verified correctness on 700-digit and 1000-digit operands vs bun.

→ **Phase 1 closed** (BigInt arithmetic surface complete). Scoreboard re-run is V3-11.
...
- [x] **V3-07** `as` cast parser support — shipped `fba2b98` (381/0/1). New `Expr::As` AST variant; postfix-precedence parse so `arr.push(x as any)` parses cleanly. Typecheck honors the asserted type via `resolve_type_ann_full`; ssa_lower forwards the inner operand (cast is identity at runtime, Any-box happens at the surrounding assignment / push site).
...
- [x] **V3-10** Auto-trigger threshold + main-exit drain — shipped `4146d79` (threshold) + `508c7c0` (cycle_unbuffer + exit-drain) (383/0/1). `cycle_buffer` triggers `cycle_collect` synchronously at 1024 entries. `__torajs_cycle_unbuffer` scrubs `p` from the buffer before any free path; called from the C-side `__torajs_obj_drop` (migrated out of inkwell IR — IR-level call triggered an LLVM 22 -O3 LTO SIGTRAP miscompile, single-TU C call sidesteps it). Main exits via `__torajs_cycle_at_exit_drain` so cycles accumulated during program lifetime are freed at exit instead of leaking.
...
- [x] **V3-12** OBSOLETE — rpn-eval already alloca-promoted (escape_analyze_array_literals + ssa_lower's on_stack path landed pre-V3). 2nd HEAD `858e974` sweep shows rpn-eval 0.952x rust (winning, within noise). Only remaining vs-rust loss is fifo-queue-100k 1.147x (algorithmic ceiling: 0.27ms gap on a 1.84ms baseline = startup noise; theoretical min per perf-debt memory ~0.24ms).
