...
| **P2 — Layer 1** | `torajs-rc`, `torajs-anyvalue`, `torajs-throw`, `torajs-ucd` (foundation) | 1–2 weeks | P2.1 (ucd) **DONE** `cba6a55`; P2.2 (rc) **DONE** `a446c1a` — Rust libtorajs_rc.a links into every `tr build` user binary; P2.3-a (anyvalue scaffold + AnyBox alloc/unbox/drop/payload_rc_inc) **DONE** `cc9f819`; P2.3-b (strict equality — payload_eq + any_any_strict_eq + any_strict_eq) **DONE** `b04d98c`; P2.3-c (any_to_str ToString coercion) **DONE** `a1b5a11`; P2.3-d.1 (any_to_number — ToNumber per ES §7.1.4 + Layer-1 link-wiring bug fix: torajs-core/build.rs now always reruns via non-existent sentinel so the staticlib copy step never goes stale, and conformance runner switched to `--workspace` so the staticlib half of every Layer-1+ sub-crate actually gets rebuilt — previously `-p torajs-cli` only refreshed rlib deps, leaving iter `.a` stale on every sub-crate edit) **DONE** `a8807ed`; P2.3-d.2 (any_compare — `<` / `<=` / `>` / `>=` per ES §7.2.13 IsLessThan + §13.10, idiomatic Rust with `CompareOp` enum + `Ordering`-based dispatch + lexicographic byte-compare for Str-Str path + embed-via-env-var refactor: lib.rs `include_bytes!(env!(TORAJS_<NAME>_STATICLIB_PATH))` replaces OUT_DIR copy, eliminating the build-script-races-staticlib-emit race that pinned stale `.a` bytes into `tr`) **DONE** `598913d`; P2.3-d.3 (any_arith — `-`, `*`, `/`, `%` per ES §13.6–§13.9, idiomatic Rust with `ArithOp` enum + `apply` / `allows_i64_fast_path` / `tag_is_i64_shaped` predicates extracted; integer fast-path preserved bit-for-bit) **DONE** `8eb66e2`; P2.3-d.4 (any_add — `+` per ES §13.15.3, Str-concat path delegates to still-C `__torajs_str_concat`/`__torajs_str_drop`; numeric path reuses `tag_is_i64_shaped` + new `alloc_number_i64` / existing `alloc_number_f64` helpers) **DONE** `90d4757` — **P2 — Layer 1 any-value family 100% Rust**; P2.4-a (torajs-throw scaffold — native-error factory registry + `__torajs_throw_range_error`/`__torajs_throw_type_error` cross-TU wrappers + internal `throw_native` dispatch, with `AtomicPtr<()>` slot array replacing C static fn-ptr table) **DONE** `0342901`; P2.4-b (LLVM-IR-emitted throw-slot machinery `__torajs_throw_set`/`_check`/`_take`/`_take_tag` → Rust statics + extern "C" fns; ssa_inkwell `define_throw_*` and `ensure_throw_globals` IR-builders deleted) **DONE** this commit — **P2 — Layer 1 fully closed**; P2.4 (throw) queued |
