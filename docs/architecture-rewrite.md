...
| **P2 — Layer 1** | `torajs-rc`, `torajs-anyvalue`, `torajs-throw`, `torajs-ucd` (foundation) | 1–2 weeks | P2.1 (ucd) **DONE** `cba6a55`; P2.2 (rc) **DONE** `a446c1a` — Rust libtorajs_rc.a links into every `tr build` user binary; P2.3-a (anyvalue scaffold + AnyBox alloc/unbox/drop/payload_rc_inc) **DONE** `cc9f819`; P2.3-b (strict equality — payload_eq + any_any_strict_eq + any_strict_eq) **DONE** `b04d98c`; P2.3-c (any_to_str ToString coercion) **DONE** `a1b5a11`; P2.3-d (any arith / compare / to_number / add) NEXT — last ~250 lines of any-value C in runtime_str.c; P2.4 (throw) queued |
