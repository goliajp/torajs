...
| **F.12** Tier 2 决策点 | — | — | 决断 playground-api: 保留/移出/strip down | F.11 ship + takagi 决断 |
| **F.13a** (if 保留) torajs-net-mini + torajs-http-mini | `torajs-net-mini` · `torajs-http-mini` | `axum` + `tower*` | metal-level minimal HTTP server over std::net | F.12 决"保留"或"strip" |
| **F.13b** (if 保留) torajs-task | `torajs-task` | `tokio 1` | async runtime + executor (可能与 A.L4 torajs-microtask 共享 reactor) | F.13a ship |
| **F.13c** (if 移出) playground-api 移出 repo | — | — | git rm + 移至 product repo | F.12 决"移出" |
| **F.14** torajs-llvm-bind (可选) | `torajs-llvm-bind` | `inkwell 0.9` | 自家 LLVM C-API binding, 精确控制 API subset, 去 inkwell 抽象层 | F.13* ship 后 takagi 主动决断 |
